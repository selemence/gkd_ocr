...
for (let i=0;i<4;i++){
    if(location.protocol.length === 5){s=''}else {s='s'}
    work = new Worker("http" + s + "://" + location.host + '/static/challenge/worker/w.js');
    work.postMessage((i*25000).toString() + '|' + ((i+1)*25000).toString() + '|' + data.r + '|' + data.c)
    work.onmessage = (e) => {
        call2(window.num, window.c, window.r, window.t, window.uuid, e.data)
    };}
    ...
    var h = {
      ...
      (function () {
        ...
        1: [function (X, l, s) {
          ...
          var O = [];
          var Z = [];
          var K = typeof Uint8Array !== "undefined" ? Uint8Array : Array;
          ...
          (function (O) {
            (function () {
              ...
              */
              ...
              function e(XX) {
                var Xl = XX.length < 0 ? 0 : r(XX.length) | 0;
                var Xs = W(Xl);
              ...
              function f(XX, Xl, Xs) {
                if (Xl < 0 || XX.byteLength < Xl) throw new RangeError("\"offset\" is outside of buffer bounds");
                if (XX.byteLength < Xl + (Xs || 0)) throw new RangeError("\"length\" is outside of buffer bounds");
                var Xh;
                if (Xl === undefined && Xs === undefined) Xh = new Uint8Array(XX);else if (Xs === undefined) {
                  Xh = new Uint8Array(XX, Xl);
                } else {
                  Xh = new Uint8Array(XX, Xl, Xs);
                }
              ...
              function Q(Xc, XU, XL) {
                ...
                while (XT < XL) {
                  var XD = Xc[XT];
                  var Xq = null;
                  var Xe = XD > 239 ? 4 : XD > 223 ? 3 : XD > 191 ? 2 : 1;
                  if (XT + Xe <= XL) {
                    var Xf;
                    var XJ;
                    var Xr;
        ...
        3: [function (X, l, s) {
          ...
          s.write = function (O, Z, K, v, M, G) {
            ...
            var N = (1 << L) - 1;
            var q = N >> 1;
            var T = M === 23 ? Math.pow(2, -24) - Math.pow(2, -77) : 0;
            var D = v ? 0 : G - 1;
            var o = v ? 1 : -1;
